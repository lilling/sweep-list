...
export class SweepsService extends BaseService {
    ...
    winOrUnwinSweep(win_action: string, sweepId: number, prizeValue?: number, thanked?: boolean){
        return this.get<user_sweep>(`${win_action}_sweep/${sweepId}` + ((win_action == 'win') ? `/${thanked}/${prizeValue}` : ``)).map(res => {
            this.fixSweepTypes(res);
            return res;
        });
    }
