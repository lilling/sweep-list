...
// TODO: why returns targrt when error?
    async SimpleMerge(source: user_account, target: user_account): Promise<user_account> {
        const db = DbGetter.getDB();
        try {
            await db.tx('simple-merge', simple_merge => {
                return this.inner(simple_merge, source, target);
            });
            return target;
        } catch (error) {
    ...
    async inner(simple_merge, source: user_account, target: user_account) {
        let q =
            `UPDATE sweepimp.google_account\n` +
            `   SET user_account_id = $<user_account_id_target>\n` +
            ` WHERE user_account_id = $<user_account_id_source>`;

        const data = await simple_merge.none(q, {
        ...
        await simple_merge.none(q, source);
        return data;

    }

// TODO: why returns target when error?
...
// TODO: change to dynamic login provider
                ...
                if (!facebook_account.user_account_id || data.user_account_id === facebook_account.user_account_id) {
                    ...
                    if (Date.now() - cookie_data.created.getTime() <= five_minutes + two_hours) {
                        ...
                        if (cookieFBExists.count > 0) {
                            ...
                            return this.ComplicatedMerge(cookie_data, data);
                        }
                    }
                }
            } else { // new FB user
